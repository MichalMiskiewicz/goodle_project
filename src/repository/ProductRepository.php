...
class ProductRepository extends Repository
{
    ...
    public function like(int $id){
        ...
        $stmt2 = $this->database->connect()->prepare('INSERT INTO public.users_product_statistics (id_user, id_product, "like") VALUES (:iduser, :id, 1)');
    ...
    public function disLike(int $id){
        ...
        $stmt2 = $this->database->connect()->prepare('INSERT INTO public.users_product_statistics (id_user, id_product, dislike) VALUES (:iduser, :id, 1)');
    ...
    public function checkLikeDislike(int $id, string $x): ?array{
        if($x == "like"){
            $stmt = $this->database->connect()->prepare('SELECT * FROM public.users_product_statistics WHERE id_user = :iduser AND id_product = :id AND "like" = 1;');
        }else{
            $stmt = $this->database->connect()->prepare('SELECT * FROM public.users_product_statistics WHERE id_user = :iduser AND id_product = :id AND dislike = 1;');
        }
        $id_user = (int)explode('@id', $_COOKIE['accept'])[1];
        $stmt->bindParam(':iduser', $id_user, PDO::PARAM_INT);
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
    ...
    public function deleteStatisticRowAndUpdateProduct(int $id, string $x){
        $stmt = $this->database->connect()->prepare('DELETE FROM public.users_product_statistics WHERE id_user = :iduser AND id_product = :id;');
        if($x == "like"){
            $stmt2 = $this->database->connect()->prepare('UPDATE public.product SET "like" = "like" -1 WHERE id = :id');
        }else{
            $stmt2 = $this->database->connect()->prepare('UPDATE public.product SET "dislike" = dislike -1 WHERE id = :id');
        }
